...
viewsNum = WebDriverWait(driver, 20).until(EC.visibility_of_all_elements_located((By.XPATH, "//span[@class='view-count style-scope yt-view-count-renderer']")))[0].text
viewsNum = int(re.sub('\D', '', viewsNum))
print("VIEWS: ", str(viewsNum))
driver.execute_script("window.scrollTo(0, 200)")
commentCount = WebDriverWait(driver, 20).until(EC.visibility_of_all_elements_located((By.XPATH, "//yt-formatted-string[@class='count-text style-scope ytd-comments-header-renderer']")))[0].text
commentCount = int(re.sub('\D', '', commentCount))
print("Comment Count: ", str(commentCount))

likeCount = WebDriverWait(driver, 20).until(EC.visibility_of_all_elements_located((By.XPATH, "//yt-formatted-string[@class='style-scope ytd-toggle-button-renderer style-text' and contains(@aria-label, 'gostaram')]")))[0].get_attribute("aria-label")
likeCount = int(re.sub('\D', '', likeCount))
print("likeCount: ", str(likeCount))

dislikeCount = WebDriverWait(driver, 20).until(EC.visibility_of_all_elements_located((By.XPATH, "//yt-formatted-string[@class='style-scope ytd-toggle-button-renderer style-text' and contains(@aria-label, 'marcações \"Não gostei\"')]")))[0].get_attribute("aria-label")
dislikeCount = int(re.sub('\D', '', dislikeCount))
print("dislikeCount: ", str(dislikeCount))
'''
#/html/body/ytd-app/div/ytd-page-manager/ytd-watch-flexy/div[4]/div[1]/div/div[7]/div[2]/ytd-video-primary-info-renderer/div/div/div[3]/div/ytd-menu-renderer/div/ytd-toggle-button-renderer[2]/a/yt-formatted-string
...
data_basics["yt_stats"] = yt #data_basics.apply(lambda x: youtube_request(x["originalTitle"]),axis=1)
